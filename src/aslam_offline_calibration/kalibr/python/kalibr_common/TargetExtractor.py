...
def extractCornersFromDataset(dataset, detector, multithreading=False, numProcesses=None, clearImages=True, noTransformation=False):
    ...
    #single threaded implementation
        ...
        for timestamp, image in dataset.readDataset():
            # Ensure image is a proper numpy array with C-contiguous layout for Eigen binding
            img_array = np.ascontiguousarray(image, dtype=np.uint8)
            # Try calling without timestamp first (numpy-to-eigen conversion issue workaround)
            try:
                if noTransformation:
                    success, observation = detector.findTargetNoTransformation(img_array)
                else:
                    success, observation = detector.findTarget(img_array)
                # Manually set the timestamp if successful
                if success == 1:
                    observation.setTime(timestamp)
            except (TypeError, RuntimeError) as e:
                sm.logWarn("Failed to call findTarget without timestamp, trying with timestamp: {}".format(e))
                if noTransformation:
                    success, observation = detector.findTargetNoTransformation(timestamp, img_array)
                else:
                    success, observation = detector.findTarget(timestamp, img_array)
            if clearImages:
