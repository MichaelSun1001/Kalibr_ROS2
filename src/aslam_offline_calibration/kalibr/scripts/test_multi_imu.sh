...
# Check if kalibr is installed
if ! ros2 pkg list | grep -q kalibr; then
    echo "Error: kalibr package not found"
    echo "Please build the package first:"
    echo "  cd kalibr_ros2"
    echo "  colcon build --packages-select kalibr"
    exit 1
fi
...
PKG_SHARE=$(ros2 pkg prefix kalibr)/share/kalibr
...
echo "------------------------------------------------------"
cat << 'EOF'
ros2 launch kalibr calibrate_multi_imu.launch.py \
